...
export default function ProductPage() {
  ...
  const FilterSection = ({
    ...
    <div className="mb-6">
      ...
      {expandedSections[section] && (
        <div
          className={`space-y-2 pl-1 overflow-y-auto max-h-48 ${
            items.length > 8 ? "scrollbar-thin" : ""
          }`}
        >
          {items.map((item) => (
            <div key={item._id} className="flex items-center space-x-2">
              ...
              />
              <label htmlFor={item._id} className="text-gray-700">
                {item.name}
              </label>
            </div>
          ))}
  ...
  return (
    <div className="relative flex flex-col lg:flex-row min-h-screen bg-gradient-to-br from-[#f8fafc] to-[#eef2f7] mt-20">
