...
const OrderPage = () => {
    ...
    useEffect(() => {
        ...
        const fetchOrders = async () => {
            try {
                setLoading(true);
                const response = await axios.get(`${API_URI}/order/my-orders`, {
    ...
    const cancelOrder = async (orderId) => {
        try {
            const response = await axios.patch(`${API_URI}/order/cancel/${orderId}`, {
                            ...
                            >
                                ...
                                <div className="p-6">
                                    <div className="flex flex-col md:flex-row justify-between items-start md:items-center">
                                        <div className="space-y-2">
                                            <div className="flex items-center space-x-3">
                                                <span className="text-xl font-bold text-white">{order.orderId}</span>
                                            ...
                                            <p className="text-gray-400">
                                                Ordered on {new Date(order.statusHistory?.[0]?.timestamp || order.date).toLocaleDateString('en-US', {
                                                    year: 'numeric',
                                                    month: 'long',
                                                    day: 'numeric'
                                                })}
                                    ...
                                    <div className="mt-6 flex items-center justify-between">
                                        ...
                                        >
                                            <span>{expandedOrder === (order._id || order.orderId) ? 'Show Less' : 'Show Details'}</span>
                                            {expandedOrder === (order._id || order.orderId) ? <FaChevronUp /> : <FaChevronDown />}
                                        </button>
                                    </div>
                                </div>
                                    ...
                                    <div className="border-t border-gray-700 bg-gray-800/50 p-6 space-y-6">
                                        ...
                                        <div className="grid gap-4">
                                            {order.items.map((item, index) => (
                                                <div key={item.id || index} className="flex items-center space-x-4 bg-gray-700/30 p-4 rounded-xl">
                                                    <img 
                                                        src={getImageSource(item)}
                                                        alt={item.name || 'Product'}
