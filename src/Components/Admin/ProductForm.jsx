...
const ProductForm = ({
  ...
  useEffect(() => {
    if (editingProduct) {
      ...
      if (
        editingProduct.linkedProducts &&
        editingProduct.linkedProducts.length > 0
      ) {
        setLinkedProducts(editingProduct.linkedProducts);
        setExpandedForms(
          new Array(editingProduct.linkedProducts.length).fill(false)
        );
  ...
  const handleLinkedProductAdd = () => {
    const cleanedImages = currentLinkedProduct.images.filter(
      (img) => img && img.trim() !== ""
    );

    if (
      !currentLinkedProduct.name ||
      !currentLinkedProduct.price ||
      !currentLinkedProduct.color ||
      !currentLinkedProduct.description ||
    ...
    setLinkedProducts([
      ...linkedProducts,
      { ...currentLinkedProduct, images: cleanedImages },
    ]);
  ...
  const handleFinalSubmit = (e) => {
    ...
    const isValidLinkedProducts = finalData.linkedProducts.every(
      (product) =>
        product.name &&
        product.price &&
  ...
  const handleImageUpload = async (e) => {
    ...
    try {
      const res = await fetch(`${API_URI}/plist/upload-product-images`, {
        method: "POST",
        body: formData,
      });
      ...
      setCurrentLinkedProduct({
        ...currentLinkedProduct,
        images: [...currentLinkedProduct.images, ...data.images],
      });
    } catch (err) {
  ...
  const handleImageRemove = (index) => {
    const updatedImages = currentLinkedProduct.images.filter(
      (_, i) => i !== index
    );
    setCurrentLinkedProduct({
      ...currentLinkedProduct,
                ...
                <div className="space-y-2">
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="grid grid-cols-3 gap-3">
                      {currentLinkedProduct.images.map((img, index) => (
